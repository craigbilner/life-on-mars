...
/******/ ([
  ...
  /***/ function(module, exports, __webpack_require__) {
    ...
    var _robots = __webpack_require__(161);

    var _mars = __webpack_require__(162);

    var _robotConfig = __webpack_require__(163);
    ...
    var App = (function (_Component) {
      ...
      _createClass(App, [{
        ...
        value: function handleChange(evt) {
          var _evt$currentTarget$va = evt.currentTarget.value.split('\n');

          var _evt$currentTarget$va2 = _toArray(_evt$currentTarget$va);

          var surfaceDimensions = _evt$currentTarget$va2[0];

          var robots = _evt$currentTarget$va2.slice(1);

          this.setState({
            surfaceDimensions: surfaceDimensions,
            robots: robots.filter(function (line) {
              return line;
            })
          });
        }
        ...
        value: function handleClick() {
          var _state$surfaceDimensi = this.state.surfaceDimensions.split(' ');

          var _state$surfaceDimensi2 = _slicedToArray(_state$surfaceDimensi, 2);

          var x = _state$surfaceDimensi2[0];
          var y = _state$surfaceDimensi2[1];

          var surface = (0, _mars.createSurface)({
            x: parseInt(x, 10),
            y: parseInt(y, 10)
          });

          var control = (0, _robots.missionControl)({
            moveMap: _robotConfig.moveMap,
            rotationMap: _robotConfig.rotationMap,
            compassRotationMap: _robotConfig.compassRotationMap,
            rotationCompassMap: _robotConfig.rotationCompassMap,
            surface: surface
          });

          var mappedRobots = this.state.robots.reduce(function (robotList, line, indx) {
            var newList = robotList.slice(0);
            var isPosition = indx % 2 == 0;
            ...
            if (isPosition) {
              ...
              var origX = _line$split2[0];
              var origY = _line$split2[1];
              var origOrient = _line$split2[2];

              newList.push({
                id: robotList.length + 1,
                origX: parseInt(origX, 10),
                origY: parseInt(origY, 10),
                origOrient: origOrient
              });
            } else {
          ...
          var processedRobots = control(mappedRobots);
          var result = Object.keys(processedRobots).map(function (key) {
            if (key !== 'scent') {
        ...
        value: function render() {
          return _react2.default.createElement(
            ...
            _react2.default.createElement(
              ...
              this.state.result.map(function (_ref) {
                var id = _ref.id;
                var curX = _ref.curX;
                var curY = _ref.curY;
                var curOrient = _ref.curOrient;
                var isLost = _ref.isLost;

                var lostText = isLost ? 'LOST' : '';
                var content = curX + ' ' + curY + ' ' + curOrient + ' ' + lostText;
                return _react2.default.createElement(
                  'div',
                  { key: id },
                  content
  ...
  /***/ function(module, exports) {
    ...
    var missionControl = exports.missionControl = function missionControl(_ref) {
      var moveMap = _ref.moveMap;
      var rotationMap = _ref.rotationMap;
      var compassRotationMap = _ref.compassRotationMap;
      var rotationCompassMap = _ref.rotationCompassMap;
      var surface = _ref.surface;
      return function (robots) {
        return robots.reduce(function (result, robot) {
          // eslint-disable-line indent
          var id = robot.id;
          var origX = robot.origX;
          var origY = robot.origY;
          var origOrient = robot.origOrient;
          ...
          var robotRoute = robot.instructions.split('').reduce(function (route, instruction) {
            var thisRoute = route.slice(0);

            if (route[route.length - 1] === 'LOST') {
              return thisRoute;
            }

            var movement = moveMap.get(instruction);
            var rotation = rotationMap.get(instruction);
            var _route = route[route.length - 1];
            var curX = _route.x;
            var curY = _route.y;
            var curOrient = _route.orient;
            ...
            if (movement) {
              ...
              var curRotation = compassRotationMap.get(curOrient);
              var newRotation = curRotation + rotation;
              var adjRotation = 0;
              ...
              var newOrient = rotationCompassMap.get(adjRotation);
              thisRoute.push({
                x: curX,
                y: curY,
                orient: newOrient
          ...
          var isLost = false;
          var lastPosition = robotRoute[robotRoute.length - 1];
          ...
          var curX = _lastPosition.x;
          var curY = _lastPosition.y;
          var curOrient = _lastPosition.orient;
  ...
  /***/ function(module, exports) {
    ...
    var createSurface = exports.createSurface = function createSurface() {
      ...
      var surface = new Array(y + 1);
      var row = new Array(x + 1);
      row.fill(1);
      surface.fill(row);
  ...
  /***/ function(module, exports) {
    ...
    var moveMap = exports.moveMap = new Map([['F', {
      x: 0, y: 1
    }]]);

    var rotationMap = exports.rotationMap = new Map([['L', -90], ['R', 90]]);

    var compassRotationMap = exports.compassRotationMap = new Map([['N', 0], ['E', 90], ['S', 180], ['W', 270]]);

    var rotationCompassMap = exports.rotationCompassMap = new Map([[0, 'N'], [90, 'E'], [180, 'S'], [270, 'W']]);
